...
def parent_selection(population_set, fitness_list, numChildren):
    ...
    # CJ - Probability of selection should be related to goodness of fitness list score
    # Need low scores to have higher probability
...
def crossover_rate(prog_a, prog_b):
    ...
    j = 0
    for city in prog_b:
        if city not in cross_over_elements:
...
coordinates_list = [[x, y] for x, y in zip(np.random.rand(n_cities)*10, np.random.rand(n_cities)*10)]
...
for k in range(generation_num):
    ...
    new_population_set = np.concatenate((initial_population_set[ranking[0:(num_chromosomes-numChildren)],:], offspring_after_crossover), axis=0)
